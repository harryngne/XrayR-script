...
#        return 0
#    fi
    bash <(curl -Ls https://raw.githubusercontent.com/harryngne/XrayR-script/master/install.sh) $version
    if [[ $? == 0 ]]; then
...
config() {
    ...
    case $? in
        0)
            echo -e "Trạng thái XrayR: ${green}đã khởi động${plain}"
            ;;
        1)
...
update_shell() {
    wget -O /usr/bin/XrayR -N --no-check-certificate https://raw.githubusercontent.com/harryngne/XrayR-script/master/XrayR.sh
    if [[ $? != 0 ]]; then
        echo ""
...
show_status() {
    ...
    case $? in
        0)
            echo -e "Trạng thái XrayR: ${green}đã khởi dộng${plain}"
            show_enable_status
            ;;
        1)
...
show_menu() {
    echo -e "
  ${green}XrayR Các tập lệnh quản lý phụ trợ，${plain}${red}không áp dụng docker${plain}
--- https://github.com/harryngne/XrayR-script ---
