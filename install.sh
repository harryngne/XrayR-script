...
# check root
[[ $EUID -ne 0 ]] && echo -e "${red}Lỗi：${plain} Tập lệnh này phải được chạy với quyền root！\n" && exit 1
...
install_acme() {
    curl https://raw.githubusercontent.com/harryngne/XrayR-script/master/acme.sh | sh
}
...
install_XrayR() {
    ...
    if  [ $# == 0 ] ;then
        last_version=$(curl -Ls "https://api.github.com/repos/harryngne/XrayR-Hinet/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        ...
        wget -N --no-check-certificate -O /usr/local/XrayR/XrayR-linux.zip https://github.com/harryngne/XrayR-Hinet/releases/download/${last_version}/XrayR-linux-${arch}.zip
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/harryngne/XrayR-Hinet/releases/download/${last_version}/XrayR-linux-${arch}.zip"
    ...
    file="https://raw.githubusercontent.com/harryngne/XrayR-script/master/XrayR.service"
    ...
    if [[ ! -f /etc/XrayR/config.yml ]]; then
        ...
        echo -e ""
        echo -e "Để cài đặt mới, vui lòng tham khảo hướng dẫn trước: https://fastpn.net, cấu hình nội dung cần thiết"
    else
        ...
        if [[ $? == 0 ]]; then
            ...
            echo -e "${red}XrayR có thể không khởi động được, vui lòng sử dụng nhật ký XrayR để kiểm tra thông tin nhật ký sau này, nếu không khởi động được, định dạng cấu hình có thể đã bị thay đổi, vui lòng vào wiki để kiểm tra：https://github.com/XrayR-project/XrayR/wiki${plain}"
        fi
    fi
    ...
    curl -o /usr/bin/XrayR -Ls https://raw.githubusercontent.com/harryngne/XrayR-script/master/XrayR.sh
